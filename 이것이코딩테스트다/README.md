...
그리고 방향 그래프에서의 사이클 여부는 DFS 를 이용하여 판별할 수 있다.
...
트리 자료구조를 이용해 집합을 표현하고, 서로소 집합을 계산하는 알고리즘.
...
참고로, 무방향 그래프 내에서의 사이클을 판별할 때 서로소 집합을 활용할 수 있다.

### 크루스칼 알고리즘

**신장 트리**: 하나의 그래프가 있을 때 모든 노드를 포함하면서 사이클이 존재하지 않는 부분 그래프를 의미한다.

**예시**: N개의 도시가 존재하는 상황에서 두 도시 사이에 도로를 놓아 전체 도시가 서로 연결될 수 있게 도로를 설치하는 문제에서 활용한다.

**알고리즘**

그리디 알고리즘에 속하고 앞서 살펴본 union-find 연산을 사용한다.

1. 간선 데이터를 비용에 따라 오름차순으로 정렬한다.
2. 간선을 하나씩 확인하며 현재의 간선이 사이클을 발생시키는지 확인한다.
   - 사이클이 발생하지 않는 경우 최소 신장 트리에 포함시킨다.
   - 사이클이 발생하는 경우 최소 신창 트리에 포함시키지 않는다.
3. 모든 간선에 대하여 2번의 과정을 반복한다.

**시간 복잡도**

간선 개수가 E개일 때, **O(ElogE)**

왜냐하면 크루스칼 알고리즘에서 시간이 가장 오래 걸리는 부분은 간선을 정렬하는 작업이므로 유의미하고
...
큐에서 빠져나온 순서대로 노드를 출력하면 그것이 위상 정렬을 수행한 결과가 된다. 위상 정렬의 답안은 여러 가지가 될 수 있다는 점이 특징이다.

**시간 복잡도**
위상 정렬의 시간 복잡도는 노드와 간선을 모두 확인한다는 측면에서 O(V+E)이다.

차례대로 모든 노드를 확인하면서, 해당 노드에서 출발하는 간선을 차례대로 제거해야 하기 때문이다.
